...
Rows in this run: 1 (moderate=1). The row is target2 for
`source_mismatch_ringdown050_noise10_seed3524578`, and the bar corresponds to
a base objective radius margin of 5.691824e-04.

Weak update-case rows to inspect first: none.

Broad radius-ambiguity rows to inspect first: none.

Interpretation for this run: the base confidence figure supports accepting the
exact target2 update at 5 sources and Tx/Rx=60. The separate objective
diagnostic CSV still carries an early_high caveat, so this figure should be
read as base-confidence evidence rather than a claim that every diagnostic
variant is clean.
